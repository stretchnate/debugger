<?
	function error($var, $str_left="", $str_right="") {
		...
		error_log($error, 3 ,'/tmp/dbug_error.log');
	}

	function clearLog($log = "/tmp/dbo.log") {
	...
	function debugOut($var, $mode = "a", $indent = "\n") {
		if( is_array($var) || is_object($var) ) {
			pre($var, $mode, "/tmp/dbo.log", $indent);
		}
		else {
			writeToFile($var, $mode, '/tmp/dbo.log', $indent);
		}
	}
	 ...
	 * @author dnate
	 */
	function pre(&$array, $mode = 'w', $filename = "/tmp/arr.log", $indent = "\r\n") {
	 ...
	 * @author dnate
	 */
	function errDump($var, $mode = 'w', $file = '/tmp/arr.log') {
		ob_start();
	 ...
	 * @author dnate
	 */
	function writeToFile($string, $mode = 'a', $filename = '/tmp/dbo.log', $indent = "\n") {
		if(is_array($string)) {
			if($filename == "/tmp/dbo.log") {
				$filename = "/tmp/arr.log";
			}
	...
	* @author dnate
	*/
	function wtf($string, $mode = 'a', $filename = '/tmp/dbo.log', $indent = "\n") {
